...
class MAAC_DIV(object):
    ...
    def update_critic(self, sample, soft=True, logger=None, **kwargs):
        ...
        for target_pi,pi, ob,ava_next,m_next in zip(self.target_policies,self.policies, next_obs,avail_action_next,mask_next):
            ...
            curr_next_target_logit = target_pi(ob)
            curr_next_logit = pi(ob)

            curr_next_pi = F.softmax(curr_next_logit,dim = -1)
            curr_next_target_pi = F.softmax(curr_next_target_logit,dim = -1)
            ...
            m_acs = m_next.unsqueeze(1).repeat(1,curr_next_pi.shape[1])
            sample_prob = curr_next_pi
            sample_prob[m_acs == 0] = 1

            curr_next_ac = Categorical(sample_prob).sample()
    ...
    def update_policies(self, sample, soft=True, logger=None, **kwargs):
        ...
        for a_i, pi, target_pi, ob,ava,m in zip(range(self.nagents), self.policies, self.target_policies, obs,avail_action,mask):
            ...
            log_probs = get_log_pi(_probs,ava,m)

            # _log_pi = (curr_ac * _log_probs).sum(dim = 1).view(-1,1)
            curr_ac_int = curr_ac_int.unsqueeze(1)
            target_log_pi = torch.gather(target_log_probs,dim = 1 ,index = curr_ac_int)

            probs = probs.cpu()
            # probs[ava == 0] = 0.0
            # print('probs_before = {}'.format(probs))
            probs = probs * ava
            # print('probs_after = {}'.format(probs))
            ...
            # print('probs_sum_shape = {}'.format(probs.sum(dim = -1,keepdim = True).shape))
            ...
            # print('pi_taken_1 = {}'.format(pi_taken))
            # print('m = {}'.format(m))
        ...
        pol_loss_all = 0
        for m,a_i, probs, log_pi,log_probs,target_log_probs,target_log_pi , (q, all_q) in zip(mask,range(self.nagents), all_probs,
                                                            all_log_pis, all_log_probs, all_target_log_probs,all_target_log_pi,
                                                            critic_rets):
            all_log_diff = log_probs - target_log_probs
            bias_baseline = (all_log_diff * probs).sum(dim = 1, keepdim = True)
            bias = (log_pi - target_log_pi).detach()
            ...
            # print('pol_loss = {}'.format(pol_loss))
    ...
    def soft_update_policy(self,tau =0.001):
        for a_i,a in enumerate(self.agents):
             soft_update(a.target_policy, a.policy,tau)
