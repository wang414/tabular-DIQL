...
class RNN(nn.Module):
    ...
    def __init__(self, input_shape, args,no_rnn_tag=False):
        ...
        self.no_rnn = no_rnn_tag
    def forward(self, obs, hidden_state):
        # print('obs = {} fc1 = {}'.format(np.shape(obs),self.fc1))
        # print('hidden_state = {}'.format(np.shape(hidden_state)))
        x = F.relu(self.fc1(obs))
        # print('x = {}'.format(np.shape(x)))
        if self.no_rnn:
            h = hidden_state
            x = F.relu(self.no_rnn_fc(x))
            q = self.fc2(x)
        else:
...
class lin_QMixer(nn.Module):
    def __init__(self, args):
        super(lin_QMixer, self).__init__()
        ...
        # self.dmac_tag = getattr(self.args, "dmac_policy", True)
        self.dmac_tag = self.args.dop_dmac_tag
    def k(self, states):
        bs = states.size(0)
        if self.dmac_tag:
...
class FOPMixer(nn.Module):
    def __init__(self, args):
        ...
        if not self.args.mat_test:
            self.unit_dim = 16
        else:
            self.unit_dim = self.state_dim
        ...
        hypernet_embed = self.args.hyper_hidden_dim
    ...
    def forward(self, agent_qs, states, actions=None, vs=None):
        ...
        weight, attend_mag_regs = self.weight(states)
        ...
        adv_tot = torch.sum(adv_q * lambda_weight, dim=1).reshape(bs, -1, 1)
        v_tot = torch.sum(weight * agent_qs + v, dim=-1).reshape(bs, -1, 1)

        return adv_tot + v_tot, attend_mag_regs

    def weight(self, states):  # attention weights from Qatten
        states = states.reshape(-1, self.state_dim)
        ...
        if not self.args.mat_test:
            ...
            unit_states = states.unsqueeze(1).repeat(1,self.n_agents,1)
        ...
        for head_key, head_selector in zip(head_keys, head_selectors):
            attend_logits = torch.matmul(head_selector.reshape(-1, 1, self.embed_dim),
                                      torch.stack(head_key).permute(1, 2, 0))
...
class QMIX_DIV:
    def __init__(self, args):
        ...
        critic_input_shape = input_shape
        if args.last_action and args.critic_no_rnn:
            critic_input_shape -= self.n_actions
        # 神经网络
        self.eval_policy_rnn = RNN(input_shape, args)  # 每个agent选动作的网络
        self.target_policy_rnn = RNN(input_shape, args)
        if self.args.dmac_zero_init:
            for target_param in self.target_policy_rnn.parameters():
                target_param.data.copy_(torch.zeros_like(target_param.data))
        self.eval_rnn = RNN(critic_input_shape, args,no_rnn_tag=args.critic_no_rnn)  # 每个agent选动作的网络
        self.target_rnn = RNN(critic_input_shape, args,no_rnn_tag=args.critic_no_rnn)

        if self.args.double_min:
            self.eval_rnn2 = RNN(critic_input_shape, args,no_rnn_tag=args.critic_no_rnn)  # 每个agent选动作的网络
            self.target_rnn2 = RNN(critic_input_shape, args,no_rnn_tag=args.critic_no_rnn)


        if self.args.mixer_module == 'dop':
            mixer_module =  lin_QMixer
        elif  self.args.mixer_module == 'fop':
            mixer_module = FOPMixer
        else:
            mixer_module = QMixNet
        self.eval_qmix_net = mixer_module(args)  # 把agentsQ值加起来的网络
        self.target_qmix_net = mixer_module(args)
        if self.args.double_min:
            self.eval_qmix_net2 = mixer_module(args)  # 把agentsQ值加起来的网络
            self.target_qmix_net2 = mixer_module(args)
        ...
        if self.args.cuda:
            ...
            if self.args.double_min:
                self.eval_rnn2.cuda()
                self.target_rnn2.cuda()
                self.eval_qmix_net2.cuda()
                self.target_qmix_net2.cuda()
        ...
        if self.args.double_min:
            self.target_rnn2.load_state_dict(self.eval_rnn2.state_dict())
            self.target_qmix_net2.load_state_dict(self.eval_qmix_net2.state_dict())
        # self.target_policy_rnn.load_state_dict(self.eval_policy_rnn.state_dict())
        ...
        if self.args.double_min:
            self.eval_parameters2 = list(self.eval_qmix_net2.parameters()) + list(self.eval_rnn2.parameters())
        if args.optimizer == "RMS":
            self.optimizer = torch.optim.RMSprop(self.eval_parameters, lr=args.lr)
            self.policy_optimizer = torch.optim.RMSprop(self.policy_parameters, lr=args.lr)
            if self.args.double_min:
                self.optimizer2 = torch.optim.RMSprop(self.eval_parameters2, lr=args.lr)
        # 执行过程中，要为每个agent都维护一个eval_hidden
        # 学习过程中，要为每个episode的每个agent都维护一个eval_hidden、target_hidden
        self.eval_hidden = None
        self.target_hidden = None
        if self.args.double_min:
            self.eval_hidden2 = None
            self.target_hidden2 = None
    ...
    def hard_update_critic(self):
        ...
        if self.args.double_min:
            hard_update(self.target_rnn2, self.eval_rnn2)
            hard_update(self.target_qmix_net2, self.eval_qmix_net2)
    ...
    def learn(self, batch, max_episode_len, train_step, epsilon=None,logger = None,policy_logger_list = None):  # train_step表示是第几次学习，用来控制更新target_net网络的参数
        ...
        q_evals_all, q_targets_all = self.get_q_values(batch, max_episode_len,no_rnn=self.args.critic_no_rnn)
        q_evals_for_q = torch.gather(q_evals_all, dim=3, index=u_for_q).squeeze(3)
        if self.args.double_min:
            q_evals_all2, q_targets_all2 = self.get_q_values(batch, max_episode_len,critic_choice=2,no_rnn=self.args.critic_no_rnn)
            q_evals_for_q2 = torch.gather(q_evals_all2, dim=3, index=u_for_q).squeeze(3)

        if not self.args.mixer_module == 'fop':
        ...
        if not self.args.check_alg:
            ...
            if self.args.mixer_module == 'fop':

                onehot_for_policy = torch.zeros(u_for_policy.squeeze(3).shape + (self.n_actions,))
                if self.args.cuda:
                    onehot_for_policy = onehot_for_policy.cuda()
                onehot_for_policy = onehot_for_policy.scatter_(3, u_for_policy, 1)

                vs = curr_action_prob * q_evals_all
                vs = torch.sum(vs, dim=-1)

                q_total_eval_for_q, attend_regs = self.eval_qmix_net(q_evals_for_q, s, u_onehot, vs)  # (32,50,1)
                if self.args.double_min:
                    vs2 = curr_action_prob * q_evals_all2
                    vs2 = torch.sum(vs2, dim=-1)

                    q_total_eval_for_q2, attend_regs2 = self.eval_qmix_net2(q_evals_for_q2, s, u_onehot, vs2)
            ...
            if self.args.mixer_module == 'fop':
                q_total_eval_for_policy , _ = self.eval_qmix_net(q_evals_for_policy, s, actions=onehot_for_policy, vs=vs)
            else:
                q_total_eval_for_policy = self.eval_qmix_net(q_evals_for_policy, s) # (32,50,1)


            # print('q_total_eval = {}'.format(q_total_eval.shape))
            if self.args.policy_train_mode == 'dop':
                coe = self.eval_qmix_net.k(s)
                q_taken_for_policy = torch.gather(q_evals_all, dim=3, index=u_for_policy).squeeze(3)
                q_baseline = (q_evals_all * curr_action_prob).sum(dim=3)
                q_adv = (q_taken_for_policy - q_baseline) * coe
            elif self.args.policy_train_mode == 'fop':
                if self.args.double_min:
                    q_evals_all_policy = torch.min(q_evals_all,q_evals_all2)
                else:
                    q_evals_all_policy = q_evals_all
                q_taken_for_policy = torch.gather(q_evals_all_policy, dim=3, index=u_for_policy).squeeze(3)
                q_baseline = (q_evals_all_policy * curr_action_prob).sum(dim=3)
                q_adv = (q_taken_for_policy - q_baseline)
            else:
                ...
                if self.args.mixer_module == 'fop':
                    acs_eye = torch.eye(self.n_actions).to(q_table.device)
                    acs_eye = acs_eye.unsqueeze(1).unsqueeze(1).repeat([1,q_table.shape[0],q_table.shape[1],1 ])
                for a_i in range(self.n_agents):
                    for ac in range(self.n_actions):
                        q_input = q_evals_for_policy.clone().detach()
                        q_input[:,:,a_i] = q_evals_all[:,:,a_i,ac]
                        if self.args.mixer_module == 'fop':
                            ac_input =onehot_for_policy.clone().detach()
                            ...
                            mix_ret, _ = self.eval_qmix_net(q_input, s, actions=ac_input, vs=vs)
                ...
                q_adv = q_total_eval_for_policy - q_baseline
            ...
            if self.args.double_min:
                q_targets2 = torch.gather(q_targets_all2, dim=3, index=u_next).squeeze(3)
        else:
            q_targets_all[avail_u_next == 0.0] = - 9999999
            q_targets = q_targets_all.max(dim=3)[0]
        if self.args.mixer_module == 'fop':
            onehot_for_q_next = torch.zeros(u_next.squeeze(3).shape + (self.n_actions,))
            if self.args.cuda:
                onehot_for_q_next = onehot_for_q_next.cuda()
            onehot_for_q_next  = onehot_for_q_next .scatter_(3, u_next, 1)

            next_vs = next_action_prob * q_targets_all
            next_vs = torch.sum(next_vs, dim=-1)
            q_total_target, _ = self.target_qmix_net(q_targets, s_next, actions=onehot_for_q_next , vs=next_vs)
            if self.args.double_min:
                next_vs2 = next_action_prob * q_targets_all2
                next_vs2 = torch.sum(next_vs2, dim=-1)
                q_total_target2, _ = self.target_qmix_net2(q_targets2, s_next, actions=onehot_for_q_next, vs=next_vs2)
                q_total_target = torch.min(q_total_target,q_total_target2)
        else:
        ...
        loss = (masked_td_error ** 2).sum() / mask.sum() + attend_regs
        self.optimizer.zero_grad()
        loss.backward()
        torch.nn.utils.clip_grad_norm_(self.eval_parameters, self.args.grad_norm_clip)
        self.optimizer.step()
        if self.args.double_min:
            td_error2 = (q_total_eval_for_q2 - targets.detach())
            # print('mask = {}'.format(mask.shape))
            masked_td_error2 = mask * td_error2  # 抹掉填充的经验的td_error
            # print('masked_td_error = {}'.format(masked_td_error.shape))
            # loss = masked_td_error.pow(2).mean()
            # 不能直接用mean，因为还有许多经验是没用的，所以要求和再比真实的经验数，才是真正的均值
            loss2 = (masked_td_error2 ** 2).sum() / mask.sum() + attend_regs2
            self.optimizer2.zero_grad()
            loss2.backward()
            torch.nn.utils.clip_grad_norm_(self.eval_parameters2, self.args.grad_norm_clip)
    ...
    def _get_inputs(self, batch, transition_idx,no_rnn = False):
        # 取出所有episode上该transition_idx的经验，u_onehot要取出所有，因为要用到上一条
    ...
    def get_q_values(self, batch, max_episode_len,critic_choice = 1,no_rnn = False):
        episode_num = batch['o'].shape[0]
        q_evals, q_targets = [], []
        for transition_idx in range(max_episode_len):
            inputs, inputs_next = self._get_inputs(batch, transition_idx,no_rnn = no_rnn)  # 给obs加last_action、agent_id
            if self.args.cuda:
                inputs = inputs.cuda()
                inputs_next = inputs_next.cuda()
                if critic_choice == 1:
                ...
                elif critic_choice == 2:
                    self.eval_hidden2 = self.eval_hidden2.cuda()
                    self.target_hidden2 = self.target_hidden2.cuda()
            if critic_choice == 1:
                q_eval, self.eval_hidden = self.eval_rnn(inputs, self.eval_hidden)  # inputs维度为(40,96)，得到的q_eval维度为(40,n_actions)
                q_target, self.target_hidden = self.target_rnn(inputs_next, self.target_hidden)
            elif critic_choice == 2:
                q_eval, self.eval_hidden2 = self.eval_rnn2(inputs,
                                                         self.eval_hidden2)  # inputs维度为(40,96)，得到的q_eval维度为(40,n_actions)
                q_target, self.target_hidden2 = self.target_rnn2(inputs_next, self.target_hidden2)
    ...
    def init_hidden(self, episode_num):
        ...
        if self.args.double_min:
            self.eval_hidden2 = torch.zeros((episode_num, self.n_agents, self.args.rnn_hidden_dim))
            self.target_hidden2 = torch.zeros((episode_num, self.n_agents, self.args.rnn_hidden_dim))
