...
class G2ANet(nn.Module):
    def __init__(self, input_shape, args):
        ...
        self.h_maac = nn.GRUCell(args.rnn_hidden_dim * 2, args.rnn_hidden_dim)
        self.action_encoding = nn.Linear(args.n_actions, args.rnn_hidden_dim)
        # Hard
        ...
        self.decoding = nn.Linear(args.rnn_hidden_dim + args.attention_dim + args.n_agents, args.n_actions)
        self.args = args
        self.input_shape = input_shape

    def forward(self, obs, hidden_state,action_input):
        size = obs.shape[0]  # batch_size * n_agents
        # 先对obs编码
        obs_encoding = f.relu(self.encoding(obs))
        h_in = hidden_state.reshape(-1, self.args.rnn_hidden_dim)
        if self.args.alg.startswith('maac'):
            acs_encoding = f.relu(self.action_encoding(action_input))
            all_encoding = torch.cat([obs_encoding,acs_encoding],dim = -1)
            h_out = self.h_maac(all_encoding,h_in)
        else:
        #     # 经过自己的GRU得到h
        ...
        # Soft Attention
        q = self.q(obs_encoding).reshape(-1, self.args.n_agents, self.args.attention_dim)  # (batch_size, n_agents, args.attention_dim)
        ...
        episode_num = size // self.args.n_agents
        agent_idx = torch.eye(self.args.n_agents).unsqueeze(0).expand(episode_num, -1, -1)
        agent_idx = agent_idx.reshape(-1,self.args.n_agents)
        if self.args.cuda:
           agent_idx = agent_idx.cuda()
        if self.args.alg.startswith('maac'):
            final_input = torch.cat([obs_encoding, x,agent_idx], dim=-1)
        else:
