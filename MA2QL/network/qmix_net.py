...
class QMixNet(nn.Module):
    def __init__(self, args):
        ...
        if self.args.check_qmix:
            self.qmix_dim = self.args.qmix_hidden_dim_for_qmix
        else:
            self.qmix_dim = self.args.qmix_hidden_dim
        if args.two_hyper_layers:
            self.hyper_w1 = nn.Sequential(nn.Linear(args.state_shape, two_layer_hidden),
                                          nn.ReLU(),
                                          nn.Linear(two_layer_hidden, args.n_agents * self.qmix_dim))
            # 经过hyper_w2得到(经验条数, 1)的矩阵
            self.hyper_w2 = nn.Sequential(nn.Linear(args.state_shape, two_layer_hidden),
                                          nn.ReLU(),
                                          nn.Linear(two_layer_hidden, self.qmix_dim))
        else:
            self.hyper_w1 = nn.Linear(args.state_shape, args.n_agents * self.qmix_dim)
            # 经过hyper_w2得到(经验条数, 1)的矩阵
            self.hyper_w2 = nn.Linear(args.state_shape, self.qmix_dim * 1)

        # hyper_w1得到的(经验条数，args.qmix_hidden_dim)矩阵需要同样维度的hyper_b1
        self.hyper_b1 = nn.Linear(args.state_shape, self.qmix_dim)
        # hyper_w2得到的(经验条数，1)的矩阵需要同样维度的hyper_b1
        self.hyper_b2 =nn.Sequential(nn.Linear(args.state_shape, self.qmix_dim),
                                     nn.ReLU(),
                                     nn.Linear(self.qmix_dim, 1)
                                     )
    ...
    def forward(self, q_values, states):  # states的shape为(episode_num, max_episode_len， state_shape)
        ...
        w1 = w1.view(-1, self.args.n_agents, self.qmix_dim)  # (1920, 5, 32)
        b1 = b1.view(-1, 1, self.qmix_dim)  # (1920, 1, 32)
        ...
        w2 = w2.view(-1, self.qmix_dim, 1)  # (1920, 32, 1)
