...
class Scenario(BaseScenario):
    def make_world(self,num_agents = 5,num_landmarks = 5,collision_penal = 0,vision = 1,rew_dist = 0.1,unit_reward = 1):
        world = World()
        # set any world properties first
        world.dim_c = 2
        self.unit_reward = unit_reward
        self.rew_dist = rew_dist
    ...
    def reward(self, agent, world):
        ...
        if agent.name == 'agent 0':
            ...
            for dist in self.min_dists:
                if dist <= self.rew_dist:
                    rew += self.unit_reward
